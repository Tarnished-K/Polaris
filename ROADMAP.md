...
**2026-07-23総括**: コードで完結するフェーズ0〜9とバックログ6.1〜6.3は実装済み。独立した「支払い・受け取り」導線をフェーズ8、LINE／Discordイベントアシスタントをフェーズ9として実装した。フェーズ8・9の3マイグレーションと3 Edge Functionはデプロイ回数を抑えた統合チェックポイントでクラウドへ反映済み。SentryはEUリージョンの実DSNを本番へ設定し、制御した実エラーの受理と秘匿化まで確認済み。残る外部環境依存項目（実機PWA・オフライン復帰、実Discord／LINE配送、Supabase WAF／Rate Limiting）は`HANDOFF.md`で別管理し、推測で完了扱いにしない。

**2026-07-24追加**: 支出メモ、支出カード上の対象者別支払い状態、金額／固定内訳の視認性向上、精算関係マップの自分中心デフォルト、支払い画面の「相手への全額／イベントを選ぶ」を実装した。イベント別支払い額は、従来の相手ごと純額を各chargeへ比例配賦し、最大剰余法で円単位の合計を親精算額へ一致させる。親精算は全対象明細の状態から導出し、既存の相手ごと相殺と外部送金境界を維持する。

**後続タスク**: 公開運用前のセキュリティ監査、実Postgres再検証、本人確認URL、デバッグ機能撤去、実機・LINE／Discord・Supabase運用受け入れは`ROADMAP2.md`で管理する。
...
Postgresマイグレーション12本が実装済み。下表は初期バックエンドを構成した先頭4本で、以降の共有参加、通知、支払い、外部アカウント連携、同名参加者、支出メモ／イベント別支払いのマイグレーションは各フェーズの実装状況に記録する。
...
3. **共有URLのセキュリティ強化**: 参加者はログイン不要で共有URL(`share_token`)のみでアクセスできる設計のため、総当たり攻撃やレートリミット未設定によるアクセス試行のリスクがある。Supabase側のRate Limiting機能、またはRPC呼び出し頻度に対する簡易的なレートリミット(例: 同一IPからの`get_event_state`呼び出し回数制限)を検討する。`share_token`の生成に十分なエントロピー(既存実装で32byteランダム値相当)が使われていることも改めて確認する。
  ...
  **Sentry本番受け入れ完了 (2026-07-23)**: EUリージョンの実DSNをNetlify production環境の`VITE_SENTRY_DSN`へ登録し、Error MonitoringとTracingだけを有効化した。Logs、Session Replay、Application Metricsは追加の収集設計が必要になるまで無効のままとする。`sendDefaultPii=false`、`dataCollection.userInfo=false`、`httpBodies=[]`に加え、エラーと10%サンプリングのtransaction双方で共有token、claim、device token、参加者名、Authorization/Cookieを再帰的にマスクする。初期表示を重くしないようSDKは初回操作または12秒後に遅延ロードし、それ以前のerror／unhandled rejectionは軽量listenerと独自Error Boundaryから即時ロードして捕捉する。SentryチャンクはPWA precache対象外とし、Lighthouse Desktop 1.00／Mobile 0.97を維持した。本番URLへ制御したエラーを発生させ、Sentry ingestの2xx受理とテスト用共有token・claim・device tokenが送信envelopeに存在しないことを確認した。Netlify Deployは`6a620c8488d5e7867e7ba54c`。
...
- GitHub Actions run `30001049555`の全工程成功後、Netlify Deploy `6a61f34636b0e4e5c55a8360`へ1回で統合反映した。本番のHTTP 200、新しい分割asset、Google OAuth開始、390px支払い画面、横あふれなし、console／page error 0件を確認した。
- SupabaseのAuth組み込みrate limitが有効であることを確認した。DB Network Restrictionsは全CIDR許可だがHTTPS APIには適用されず、固定の管理元IPもないため、CLI／マイグレーションを遮断する変更は保留した。BOT受信は署名、timestamp、リプレイ防止、HMAC化外部ID単位10回/5分でアプリ側制限済み。
...
2026-07-23、ユーザーの明示的な指示により`SettlementRelationshipMap`として実装済み。全体表示／自分中心表示、ノードフォーカス、比較カードとの連動、金額チップの非重複配置を含む。2026-07-24以降は幹事・参加者とも自分中心をデフォルトとし、全体表示は任意切替にする。選択中の人物に関係しない中立線は自分中心表示で非表示のまま維持するが、冗長な「中立の精算は非表示」という説明文は表示しない。以下の記載は当初仕様として残すが、回帰確認ではこの現行方針を優先する。
...
**現行方針 (2026-07-23)**: ユーザー確認の結果、マトリクスは分かりにくいためUIから撤去した。`SettlementMatrixView`と切替タブを削除し、精算全体の入口は関係マップへ一本化する。ペアカードと内訳は引き続き残し、正確な金額の確認に使用する。
...
### 6.4 同名参加者と狭幅スマートフォン対応 — 実装済み

**完了 (2026-07-23)**: 参加者名は同一イベント内で重複入力を許可し、先に登録した名前を維持したまま、後続を`名前(1)`、`名前(2)`の順で自動採番する。共有URL参加と幹事代理登録は同じDB関数を使用し、ローカルデモも同じ規則へ揃えた。「あなた」「幹事」の予約語拒否は継続する。

320px縦型表示をPlaywrightの常設プロジェクトへ追加し、4つの主要タブを短縮表示、支出追加を絞り込み行の上へ分離、ヘッダーと設定一覧を狭幅化、立替グラフを2本並べても切れない寸法へ調整した。支出・立替・精算・支払い・イベント設定を320pxで確認し、ページ全体の横はみ出しとconsole／page errorがないことをNetlify本番でも確認した。

### 6.5 支出メモ・対象者別状態・イベント別支払い — 実装済み

**完了 (2026-07-24)**:

- 支出へ500文字までの任意メモを追加し、ローカル／クラウドの追加・編集・取得で保持する。支出カードはメモ、拡大した総額、対象者ごとの「精算前／支払い前／支払報告済み／受取確認済み／相殺済み」等を表示する。
- 金額指定の内訳を狭幅では1列にし、参加者名と金額を拡大する。320pxでもカードと主要操作が横にはみ出さない。
- `settlement_items`へ`payable_amount`と`payment_status`を追加する。相手ごとの相殺後純額はcharge明細へ比例配賦し、端数は最大剰余法で決定的に配る。全明細の合計は必ず親`settlements.amount`と一致する。
- 支払者は「相手への全額」または未払いの支出イベントを複数選択して報告できる。受取側は報告済み明細を確認でき、親状態は明細の集約結果から`pending → reported → paid`へ進む。既存BOTの全額報告／確認は互換トリガーで全明細へ反映する。
- 自動検証はVitest 115件、Playwright 34件成功（対象外2件skip）、PGlite 12マイグレーション、本番ビルド成功。実Postgresと本番Netlifyの反映結果は`HANDOFF.md`へ記録する。
