...
## 8. 次回セッション開始用・残存タスク一覧

次回は以下の順に進める。コード側のCritical／High対応、DB migration、ホスト済みpgTAP、通知dispatcher認証、冪等性、本番反映は完了済みのため、同じ作業をやり直さない。

### P0: Android／iPad実機受け入れ

- [ ] Android ChromeとiPad SafariでPWAをホーム画面へ追加し、standalone起動を確認する。
- [ ] 2端末から同じイベントへ別参加者として入り、共有、参加、支出追加、精算確定、支払報告、受取確認のRealtime反映を確認する。
- [ ] Androidをオフラインにして支出を1件追加し、オンライン復帰後に1件だけ登録され、iPadへ反映されることを確認する。
- [ ] 指定のGoogleテストアカウントでOAuthを行い、イベントURL、共有URL、claim URLへ正しく戻ることを確認する。
- [ ] PayPay外部遷移後に安全にアプリへ戻れることを確認する。
- [ ] 端末、OS、ブラウザ、操作、結果を`HANDOFF.md`へ記録し、テストイベントを削除する。

### P0: LINE／Discord実サービス受け入れ

- [ ] 専用DiscordテストWebhookとLINEテストチャネルを用意し、資格情報をSupabase secretsだけへ登録する。
- [ ] テスト通知、精算確定、支払報告、受取確認、全員完了、1日1回催促の実配送を確認する。
- [ ] LINE Postback／Discord Interactionから、アカウント連携、状況照会、本人の支払報告・受取確認をE2E確認する。
- [ ] 配送履歴、再試行、終端失敗、重複防止、署名不正、リプレイ、期限切れ、rate limitを確認する。
- [ ] 終端状態のjobが再取得されないことと、ログへsecret、PayPay ID、個人識別子が露出しないことを確認する。
- [ ] 32bit・5分有効の連携コードについて、実サービス上の試行制限と期限切れを確認する。実測で不足があれば、コード長または失敗試行計上方式を強化する。
- [ ] 受け入れ完了後、不要なテストデータと資格情報を削除またはローテーションする。

### P1: 公開運用の受け入れ

- [ ] 固定管理元IPまたは安全なCI経路を決めてから、DB Network Restrictionsを必要なCIDRへ絞る。
- [ ] HTTPS API／Edge FunctionのWAF・Rate Limiting閾値と、遮断時の解除手順を決める。
- [ ] Auth、DB、Edge Function、Netlify、Sentryの監視項目、通知先、担当者、アラート閾値を決める。
- [ ] バックアップからの復元訓練を行い、復旧時間と手順を記録する。
- [ ] service role、連携暗号鍵、LINE、Discord、Sentry、OAuth資格情報のローテーション訓練を行う。
- [ ] インシデント時のshare token再発行、PayPay ID削除、外部連携無効化、利用者通知の手順を文書化する。
- [ ] 公開前のプライバシーポリシーとリリースチェックリストを最終化する。

### 再開順と証拠

1. Android／iPad実機受け入れ
2. LINE／Discord実サービス受け入れ
3. 公開運用の受け入れ

各項目は成功だけでなく、失敗時の画面、HTTP status、監査ログも確認し、機密情報を除いた証拠を`HANDOFF.md`へ残す。
